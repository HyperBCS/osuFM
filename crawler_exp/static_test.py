...
for m in maps.values:
    m[2] = (base64.b64encode(m[2].encode('UTF-8'))).decode("utf-8") 
    m[3] = (base64.b64encode(m[3].encode('UTF-8'))).decode("utf-8") 
    m[4] = (base64.b64encode(m[4].encode('UTF-8'))).decode("utf-8") 
    m[18] = (base64.b64encode(m[18].encode('UTF-8'))).decode("utf-8") 
    map_arr.append(m)
map_arr.sort(key=lambda x:x[19], reverse=True)
f=open("comp_maps.csv", "w")
f.write("bid,sid,name,artist,mapper,num_scores,pop_mod,avg_pp,avg_acc,avg_rank,avg_pos,mode,cs,ar,od,length,bpm,diff,version,score,calculated\n")
for m in map_arr:
