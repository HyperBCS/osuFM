...
class Beatmap{
    public:
        Beatmap(int id, int set_id, std::string title, std::string artist, std::string mapper, float cs
        , float ar, float od, int length, float bpm, float diff, std::string version, int mode){
            this->id = id;
            this->set_id = set_id;
            this->title = title;
            this->artist = artist;
...
int modsToInt(json mod_string_arr){
    ...
    // if(mod_comb_str != intToMods(mods)){
    //     std::cout << mod_comb_str << mods << intToMods(mods) << std::endl;
    // }
    return mods;
}
